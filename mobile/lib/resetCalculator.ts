...
// monthCalculator.ts/weekCalculator.ts. Keep this file in sync with
// src/lib/resetCalculator.ts if the badge thresholds/labels ever change.

// The whole village sees the same card on the same calendar date — a
// deliberate choice: mothers sharing their Reset the same day are
// naturally doing the *same* prompt in their own way, which makes browsing
// the Gallery more fun. The rotation is a plain, deterministic
// days-since-epoch index — no randomness.
...
// The next badge still ahead of her.
...
// one threshold can be crossed by any single "I'm doing this" tap.
