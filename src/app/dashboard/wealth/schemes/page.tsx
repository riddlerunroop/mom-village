...
import SchemesFilterList, { type SchemeSection, type Stage } from "@/components/SchemesFilterList";
...
//
// 2026-07-28: restructured from static JSX into tagged data (stage,
// employment type, state-scope) so SchemesFilterList can filter it — same
// exact wording as before, no facts changed, just made filterable.
const SECTIONS: SchemeSection[] = [
  {
    ...
    schemes: [
      ...
      },
      {
        ...
        employment: "any",
        scope: "national_state_variation",
      },
      {
...
export default async function SchemesPage() {
  ...
  // Best-effort stage detection for the "may apply to me" filter — the one
  // thing we actually know about her without asking anything new. Employment
  // type and state aren't collected anywhere in the app today, so those stay
  // manual filters rather than fake personalization.
  const referenceDate = profile?.baby_dob || profile?.due_date;
  const monthNumber = referenceDate ? calculateMonthNumber(referenceDate) : null;
  let currentStage: Stage | null = null;
  if (monthNumber !== null) {
    if (monthNumber < 0) currentStage = "pregnancy";
    else if (monthNumber < 12) currentStage = "postpartum";
    else currentStage = "early_childhood";
  }
    ...
    <main className="max-w-[760px] mx-auto px-6 py-10">
      ...
      {!isSubscribed ? (
        ...
        <>
          ...
          <SchemesFilterList sections={SECTIONS} currentStage={currentStage} />

          <NoteCard>
            <p className="mb-3">
              A few more worth asking about locally — rules and availability
              for these vary more by state, so treat them as a starting
              point for a conversation with your Anganwadi worker, ASHA/ANM,
              or hospital, rather than a full entry yet:
            </p>
            <BulletList
              items={[
                <>
